...
def my_softmax(values: np.ndarray, T=1.):
    ...
    # Это предотвращает переполнение при вычислении экспоненты
    max_val = np.max(scaled_val)
    shifted_val = scaled_val - max_val

    # Вычисляем экспоненты
    exp_val = np.exp(shifted_val)

    # Вычисляем вероятности (softmax)
    probas = exp_val/ np.sum(exp_val)
...
class QLearningAgent:
    ...
    def get_value(self, state):
        ...
        q_val = [self.get_qvalue(state, action) for action in possible_actions]
        value = max(q_val) if q_val else 0.0
    ...
    def update(self, state, action, reward, next_state):
        ...
        curr_q = self.get_qvalue(state, action)
        next_val = self.get_value(next_state) if next_state is not None else 0.0
        target = reward + gamma * next_val
        qvalue = (1 - learning_rate) * curr_q + learning_rate * target
    ...
    def get_best_action(self, state):
        ...
        q_val = [self.get_qvalue(state, action) for action in possible_actions]
        best_action_index = np.argmax(q_val)
    ...
    def get_action(self, state):
        ...
        prob = self.get_softmax_policy(state)
        chosen_action = np.random.choice(possible_actions, p=prob)
...
class EVSarsaAgent(QLearningAgent):
    ...
    def get_value(self, state):
        ...
        q_vals = [self.get_qvalue(state, action) for action in possible_actions]
        value = sum(prob * q_val for prob, q_val in zip(action_probs, q_vals))
        assert value is not None
